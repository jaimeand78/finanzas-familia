...
| DA-28 | **La versión visible se lee del cache, nunca se escribe a mano.** `index.html` es cache-first, así que un número hardcodeado ahí llega desactualizado al usuario desactualizado y jamás detecta nada. La etiqueta se deriva de `caches.keys()` filtrando el prefijo `organiza2-v` y ordenando **numéricamente** (un `sort()` de strings pone `v2-9` después de `v2-17`). Única fuente de verdad: `CACHE_NAME`. | ✅ Implementado |
